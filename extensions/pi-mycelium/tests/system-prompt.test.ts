...
const builtinKinds: EvolutionKindRow[] = [
	{
		name: "convention",
		definition:
			"A naming, layout, structural, or behavioral pattern for organizing or operating on the store.",
	...
	},
	{
		name: "lesson",
		definition:
			"A distilled insight from past work, intended to inform future behavior.",
		defined_at_version: "0.1.0",
		source: "builtin",
		event_count: 2,
	},
...
const sampleActiveEvents: ActiveEvolutionEvent[] = [
	{
		...
		rationale:
			"Adopting <date>-<slug>.md filenames so incidents sort chronologically.",
	},
	{
		...
		kind: "lesson",
		rationale:
			"Queries that mention `latency` correlate with deploys 80% of the time.",
	},
];
...
describe("systemPromptAvailable — active evolution section", () => {
	...
	it("truncates list at 10 entries and shows overflow footer", () => {
		const manyEvents: ActiveEvolutionEvent[] = Array.from(
			{ length: 13 },
			(_, i) => ({
				ts: "2026-05-01T00:00:00Z",
				agent_id: "a",
	...
	it("does not show overflow footer when events are exactly at the limit", () => {
		const tenEvents: ActiveEvolutionEvent[] = Array.from(
			{ length: 10 },
			(_, i) => ({
				ts: "2026-05-01T00:00:00Z",
				agent_id: "a",
...
describe("systemPromptAvailable — recording evolution section", () => {
	...
	it("mentions that evolve is metadata-only and not a second memory system", () => {
		const block = makeBlock();
		expect(block).toContain("never mutates the store");
		expect(block).toContain("not a second memory system");
	});
});
