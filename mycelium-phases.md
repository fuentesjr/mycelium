...
- The Frontier model classes the suite targets — multiple, spanning at least two providers (e.g., Anthropic and OpenAI; ideally also Gemini or open-weights) — and their expected pass rates per criterion. The cross-provider spread is what makes "model-agnostic" a measured claim rather than an assertion.
...
- CLI surface: `mycelium read`, `write`, `edit`, `ls`, `glob`, `grep`, `rm`, `mv`. Eight subcommands invoked through the agent harness's existing shell tool. No specialized log subcommand — the activity log is read like any other file. Every mutating subcommand accepts an optional `--expected-version` flag; concurrency is first-class from v1.
...
- **Activity log at reserved path `_activity/YYYY/MM/DD/{agent_id}.jsonl`.** Every successful mutating operation produces a JSONL entry on the writing agent's daily file. Reads are not logged. `mycelium` rejects agent writes under any `_`-prefixed root path — the one principled enforcement exception in the design. Agent reads it via `mycelium read`/`glob`/`grep` (or raw `cat`/`ls`/`rg`) like any other content. Entries bounded at 4KB; durability contract per design section 5.
- **Reference agent harness — pi.dev extension.** Phase 1 ships a TypeScript pi.dev extension as the reference integration. It registers no tools — the agent invokes `mycelium` via pi's built-in `bash` tool — and contributes only a system-prompt block (via `before_agent_start`, introducing the conventions and the eight subcommands), env-var setup on `session_start` (`MYCELIUM_AGENT_ID` defaulting to `pi-agent`, `MYCELIUM_SESSION_ID` from `ctx.sessionManager.getLeafId()`, minting new on fork), and observation of the `context` event as a passive signal recorded to the activity log with op `"context_signal"`. Default mount at `.pi/mycelium/store/` (project-scoped); switches to `~/.pi/mycelium/store/` when extension config sets `scope: "global"` (cross-project shared memory). One mode per session in Phase 1; layered project-over-global is a Phase 3 feature. Multi-agent and multi-session scenarios are exercised via pi's session/branching model plus concurrent extension instances against the same store.
...
3. **Conflict recovery on real models.** When a conditional write fails, the model receives the typed conflict error and produces sensible recovery behavior (re-read, merge, retry) given only the error and no special prompting. Verified across at least three Frontier model classes (e.g., Claude Opus 4.x, the leading GPT-5 tier, Gemini Ultra).
