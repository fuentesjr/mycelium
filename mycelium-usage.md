...
2. **The same CLI is model-agnostic.** Swapping the harness from Claude Opus 4.x to the leading GPT-5 tier or Gemini Ultra produces the same observable behavior on the same store. The nine subcommands, the activity log layout, the conflict semantics — all of it travels. The Phase 1 acceptance criteria require this property to be measured across providers, not asserted (see `mycelium-phases.md`).
