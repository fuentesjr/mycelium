...
func transactionalWrite(errOut io.Writer, id Identity, requested string, content []byte, expectedVersion string, includeContent bool) (version string, rc int) {
...
func transactionalEdit(errOut io.Writer, id Identity, requested, oldStr, newStr, expectedVersion string, includeContent bool) (version string, rc int) {
	abs, err := resolveUnderMount(id.Mount, requested)
	if err != nil {
...
func transactionalRemove(errOut io.Writer, id Identity, requested, expectedVersion string, includeContent bool) (priorVersion string, rc int) {
	abs, err := resolveUnderMount(id.Mount, requested)
	if err != nil {
...
func transactionalMove(errOut io.Writer, id Identity, src, dst, expectedVersion string, includeContent bool) (version string, rc int) {
	...
	if _, err := os.Stat(dstAbs); err == nil {
		if rc := emitDestinationExists(errOut, id.Mount, dstAbs, includeContent); rc != ExitOK {
			return "", rc
		}
