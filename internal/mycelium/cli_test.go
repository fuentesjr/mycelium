...
func TestDispatchRejectsExtraArgs(t *testing.T) {
	tests := []struct {
		...
		{"read", []string{"read", "one.md", "two.md"}},
		{"write", []string{"write", "one.md", "two.md"}},
		{"edit", []string{"edit", "one.md", "two.md", "--old", "x", "--new", "y"}},
		{"ls", []string{"ls", "extra"}},
		{"glob", []string{"glob", "*.md", "extra"}},
		{"grep", []string{"grep", "--pattern", "x", "extra"}},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, errOut, rc := runDispatch(t, tt.args...)
			if rc != ExitUsage {
				t.Fatalf("exit code: got %d, want %d (stderr=%q)", rc, ExitUsage, errOut)
			}
			if !strings.Contains(errOut, "unexpected argument") {
				t.Fatalf("stderr: got %q, want unexpected argument", errOut)
			}
		})
	}
}
